...
describe('preMatchKeyword（关键词预匹配）', () => {
    it('能匹配到完整关键词', () => {
        const result = preMatchKeyword('今天 Claude Sonnet 4.6 发布了新版本', ['Claude Sonnet 4.6', 'Claude Sonnet', 'Sonnet 4.6']);
        expect(result.matched).toBe(true);
        expect(result.matchedTerms).toContain('Claude Sonnet 4.6');
    });
    it('能匹配到关键词变体', () => {
        const result = preMatchKeyword('新版 Claude Sonnet 表现优异', ['Claude Sonnet 4.6', 'Claude Sonnet', 'Sonnet 4.6']);
        expect(result.matched).toBe(true);
        expect(result.matchedTerms).toContain('Claude Sonnet');
    });
    it('不匹配无关内容', () => {
        const result = preMatchKeyword('今天天气很好，适合出门散步', ['Claude Sonnet 4.6', 'Claude Sonnet', 'Sonnet 4.6']);
        expect(result.matched).toBe(false);
        expect(result.matchedTerms).toHaveLength(0);
    });
    it('不区分大小写', () => {
        const result = preMatchKeyword('CLAUDE SONNET 4.6 is amazing', ['claude sonnet 4.6']);
        expect(result.matched).toBe(true);
    });
});
...
describe('AI Fallback 行为（无 API Key）', () => {
    it('preMatch=true 时 fallback 给出较高默认分', async () => {
        ...
        try {
            const result = await analyzeContent('Claude Sonnet 4.6 is amazing', 'Claude Sonnet 4.6', { matched: true, matchedTerms: ['Claude Sonnet 4.6'] });
    ...
    it('preMatch=false 时 fallback 给出较低默认分', async () => {
        ...
        process.env.OPENROUTER_API_KEY = '';
        try {
            const result = await analyzeContent('今天天气真好', 'Claude Sonnet 4.6', { matched: false, matchedTerms: [] });
...
//# sourceMappingURL=aiRelevance.test.js.map