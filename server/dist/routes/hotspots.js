...
router.get('/', async (req, res) => {
    try {
        const { page = '1', limit = '20', source, importance, keywordId, isReal, timeRange, timeFrom, timeTo, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
        ...
        if (timeRange) {
            ...
            where.createdAt = {};
            if (timeFrom)
                where.createdAt.gte = new Date(timeFrom);
            if (timeTo)
                where.createdAt.lte = new Date(timeTo);
        }
        // 排序处理
        let orderBy;
        const sort = sortBy;
        ...
        let hotspots;
        if (needsMemorySort) {
            const sorted = sortHotspots(rawHotspots, sort, order);
...
router.post('/search', async (req, res) => {
    try {
        ...
        // AI 分析前几个结果
        const analyzedResults = await Promise.all(results.slice(0, 10).map(async (item) => {
            try {
