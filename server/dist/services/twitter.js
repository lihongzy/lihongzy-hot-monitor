...
//   3. 按质量评分排序
// ============================================================
function filterAndRankTweets(tweets) {
    ...
    const filtered = tweets.filter(tweet => {
        ...
        if (tweet.likeCount < minLikes * factor)
            return false;
        if (tweet.retweetCount < minRetweets * factor)
            return false;
        if (tweet.viewCount < minViews * factor)
            return false;
        if (tweet.author.followers < minFollowers * factor)
            return false;
        return true;
    });
...
// 日期工具
// ============================================================
function formatSinceDate(daysAgo) {
...
// ============================================================
function buildAdvancedQuery(keyword, type) {
    const parts = [keyword];
    // 排除 RT 和纯回复
...
// 获取单页推文（支持分页）
// ============================================================
async function fetchTweetPage(query, queryType, cursor) {
...
export async function searchTwitter(query) {
    try {
        ...
        return qualityTweets.map((tweet) => ({
