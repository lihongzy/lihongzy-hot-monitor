 ...
 */
function extractCoreTerms(keyword) {
    const terms = [];
    // 按空格、连字符、下划线分割
...
// ========== AI 内容分析（关键词感知） ==========
function buildAnalysisPrompt(keyword, preMatchResult) {
...
export async function batchAnalyze(contents, keyword, expandedKeywords) {
    // 并行分析，但限制并发数
    const batchSize = 3;
    const results = [];
    for (let i = 0; i < contents.length; i += batchSize) {
        const batch = contents.slice(i, i + batchSize);
        const batchResults = await Promise.all(batch.map(content => {
